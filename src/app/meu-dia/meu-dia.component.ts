...
@Component({
  ...
  styleUrl: './meu-dia.component.css'
})
export class MeuDiaComponent implements OnInit {
  ...
  loadTasks(): void {
    this.tasksService.getTasks('meu-dia', this.loggedUserId).subscribe({
      next: (data: any) => {
        this.tasks = data;
   ...
   toggleImportant(task: any): void {
    ...
    concluirTarefa(task: any): void {
      ...
      this.tasksService.updateTask(task.id, task.important, updatedStatus).subscribe({
        next: response => {
          console.log(`Tarefa ${task.id} concluída com sucesso:`, response); //REMOVER ESSA LINHA
